...
class Module_Wires(Game):
    ...
    def setGUI(self):
        ...
        self.other.counter = self.other.after(1000, self.other.update_timer())
    ...
    def wirePull(self):
        ...
        def check (index):
            ...
            if (connections[index] != "checked"):
                # check that the pull is correct
                if (index == self.correct):
                    print(index);
                    self.other.Module_3_Done = True;
                    self.other.MainMenu();
                else:
                    print(index);
                    self.other.strike();
                    connections[index] = "checked";
                                        
        # detect a pulled line
        for i in range(len(connections)):
            print(f"in loop at {i}")
            # dont want to double-call a previously incorrectly pulled wire
            GPIO.add_event_detect(g.inputPins[i], GPIO.BOTH, callback=lambda *a: check(i), bouncetime=300);
